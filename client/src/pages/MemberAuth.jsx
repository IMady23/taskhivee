 ...
 * Handles both login and signup for members
...
export default function MemberAuth() {
  ...
  const { login, registerMember } = useContext(AuthContext);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'login') {
        ...
        setSuccessMessage('Account created! Please check your email for OTP.');
        setTimeout(() => navigate('/member-dashboard'), 800);
      }
    } catch (error) {
      setErrors({
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center p-4 overflow-hidden">
      ...
      <motion.div
        ...
        className="w-full h-full absolute inset-0 bg-gradient-to-br from-purple-50/80 via-purple-100/80 to-pink-50/80 -z-1"
      />
      <motion.div
        initial={{ scale: 0.95 }}
        animate={{ scale: 1 }}
      ...
      >
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-purple-600 mb-2">
            Member {mode === 'login' ? 'Login' : 'Sign Up'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === 'signup' && (
            ...
            >
              ...
              <input
                ...
                className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 transition ${errors.teamCode
          ...
          <motion.button
            ...
            type="submit"
            disabled={loading}
            className="w-full py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold rounded-lg hover:from-purple-700 hover:to-pink-700 disabled:opacity-50 disabled:cursor-not-allowed transition mt-6"
          >
        ...
        <p className="text-center text-gray-600 text-sm">
          {mode === 'login' ? "Don't have a member account?" : 'Already a member?'}{' '}
          <Link
            to={mode === 'login' ? '/member-auth?mode=signup' : '/member-auth?mode=login'}
